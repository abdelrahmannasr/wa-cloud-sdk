...
and surface it as a `TemplateStatusEvent` with fields `templateId: string`, `templateName: string`, `language: string`, `status: TemplateEventStatus`, `reason?: string`, plus a `Date` timestamp computed from the enclosing `entry.time` (epoch seconds × 1000, like `MessageEvent.timestamp`).
