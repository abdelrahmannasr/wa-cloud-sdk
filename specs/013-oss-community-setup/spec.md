...
- The signing-key lifecycle (GPG/SSH key generation, secure storage, rotation cadence, revocation on compromise) is the maintainer's responsibility. The release workflow consumes the key via a repository secret and does not prescribe a rotation cadence; quarterly rotation is recommended but out of scope for this feature.
...
- Rotation cadence for `NPM_TOKEN`, `CODECOV_TOKEN`, and the signing-key secret is the maintainer's responsibility. Industry guidance (~90 day rotation) is noted as best practice but not enforced by this feature.
...
- CI wall-clock duration, observability notifications (workflow-failure email/paging), and platform availability during release windows rely on GitHub-provided defaults. This feature does not prescribe specific latency SLAs, custom notification channels, or fallback behavior during platform degradation; those are deferred to future iterations.
- CI workflow logs, coverage reports, and release artifacts follow GitHub Actions' and Codecov's default retention policies (90 days for Actions logs, per-plan for Codecov). Extending retention is out of scope for this feature.
