...
- [ ] T038 [US5] Add `./catalog` subpath resolution test to `tests/subpath-exports.test.ts` (≥2 tests): `import { Catalog } from 'wa-cloud-sdk/catalog'` resolves to a constructor; `import * as catalogMod` exposes the documented key list (`Catalog`, `CatalogResource`, `Product`, `ProductAvailability`, `CreateProductRequest`, `UpdateProductRequest`, `ListProductsParams`, `ListProductsResponse`, `ListCatalogsResponse`, `CATALOG_VALIDATION`).
